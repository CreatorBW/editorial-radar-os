...
from .nlp import clean_html_text, infer_desk
...
def fetch_rss(
    ...
    resp = requests.get(
        url,
        timeout=timeout,
        headers={"User-Agent": "EditorialRadarOS/1.0"},
    )
    ...
    for entry in feed.entries:
        ...
        title = clean_html_text(raw_title).strip()
        summary = clean_html_text(raw_summary).strip()
    ...
    for item in items:
        ...
        try:
            conn.execute(
                ...
                (
                    item["source_name"],
                    item.get("source_tier", "standard"),
                    title,
                    url,
                    summary,
                    item.get("published_at"),
                    item.get("fetched_at", utcnow_iso()),
                    item.get("content_hash") or content_hash(title, url),
                    item.get("desk_hint") or infer_desk(f"{title} {summary}"),
                ),
            )
            inserted += 1
